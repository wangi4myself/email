// use actix_web::test;
use email::configuration::{get_configuration, Application, DatabaseSettings};
use email::startup::get_connection_pool;
...
pub async fn spawn_app() -> TestApp {
    ...
    let appliction_port = server.port();
    let address = format!("http://127.0.0.1:{}", appliction_port);

    let _ = tokio::spawn(server.run_until_stopped());
    TestApp {
        address,
        port: appliction_port,
